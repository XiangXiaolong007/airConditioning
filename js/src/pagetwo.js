...
$(function () {
    ...
    $("#everyQc").click(function () {
        ...
        console.log(Ql)
        $("#Ql").append("<span style='margin-bottom: 5px;font-weight: 700;'>单位建筑基底面积空调系统年日均耗电量QL:QL = </span><span style='display:inline-block;width:50px;'>" + Ql + "</span>(A·h/d/㎡)")
        // 使用刚指定的配置项和数据显示图表。
    ...
    $("#confirm-tab").click(function () {
        ...
        if (!monthBeginTab.val() || !monthEndTab.val()) {
            ...
            eachQc.empty();
            for (var i = +monthbeginVal; i <= +monthEndVal; i++) {
                eachQc.append("<div class='form-group' style='margin-right:20px'><label for='Q" + i + "-tab'>" + i + "月</label><input type='text' class='form-control' placeholder='' id='Q" + i + "-tab' style='margin-left:0;width:80px;'></div>")
            }
            return false;
        }
    });
    ...
    $("#everyQc-tab").click(function () {
        ...
        for (var i = +monthbeginVal; i <= +monthEndVal; i++) {
            ...
            QcAssemble["Qc" + i] = (QAssemble["Q" + i] / (380 * COPTab.val() * scaleTab.val())).toFixed(2);
            option.xAxis.data.push(i + "月");
            option.series[0].data.push(QcAssemble["Qc" + i]);
            monthSum += monthArray[i];
            QcSum += +QcAssemble["Qc" + i]
        ...
        let Ql = (QcSum / monthSum).toFixed(2);
        console.log(Ql)
        $("#Ql-tab").append("<span style='margin-bottom: 5px;font-weight: 700;'>单位建筑基底面积空调系统年日均耗电量QL:QL = </span><span style='display:inline-block;width:50px;'>" + Ql + "</span>(A·h/d/㎡)")
        // 使用刚指定的配置项和数据显示图表。
